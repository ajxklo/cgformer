...
class OneHotSTE(torch.autograd.Function):
    ...
    def forward(ctx, input, num_classes):
        # 计算离散的 one-hot 编码
        one_hot_hard = F.one_hot(input.long(), num_classes=num_classes).float()
        ctx.save_for_backward(input)  # 保存 input 用于反向传播
        return one_hot_hard
    ...
    def backward(ctx, grad_output):
        ...
        # 将梯度传递回 input，这里通过输出的梯度的求和来进行
        grad_input = grad_output.sum(dim=-1)  # 对最后一维求和，以适配 input 的形状
        return grad_input, None  # num_classes 无需梯度


# 示例用法
def one_hot_ste(gt_depths, D):
    one_hot_encoded = OneHotSTE.apply(gt_depths, D + 1)  # 包含D+1个类别
    return one_hot_encoded.view(-1, D + 1)[:, 1:]  # 去掉第一个类别
...
class GeometryDepth_Net(BaseModule):
    ...
    def get_downsampled_gt_depth(self, gt_depths):
        ...
        # gt_depths = one_hot_ste(gt_depths,self.D)
    ...
    def forward(self, input, img_metas,stereo_depth):
