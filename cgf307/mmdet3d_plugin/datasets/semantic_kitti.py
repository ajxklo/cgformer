...
class SemanticKITTIDataset(Dataset):
    ...
    def get_data_info(self, index):
        ...
        flow=[]
        if info["flow_path"] :
            flow.append(readFlow(info["flow_path"][0]))
            flow.append(readFlow(info["flow_path"][1]))
        # flow.append(readFlow(info["flow_path"][1]))
        input_dict = dict(
        ...
        if len(info['stereo_depth_path']) > 1:
            img_r = int(info['img_2_path'].split("/")[-1].split(".")[0]) - 1
            img_r = str(img_r).zfill(6)
            img_r_path = os.path.join(self.data_root, "sequences", info['sequence'], 'image_2', img_r + '.png')
            image_paths.append(img_r_path)
        ...
        input_dict['img_filename2'] = info['img_3_path']
    ...
    def load_annotations(self, ann_file=None):
        ...
        for sequence in self.sequences:
            ...
            for id_path in glob.glob(id_base_path):
                ...
                stereo_depth_path = []
                stereo_depth_path.append(os.path.join(self.stereo_depth_root, "sequences", sequence, img_id + '.npy'))

                if not os.path.exists(voxel_path):
                    voxel_path = None
                if int(img_id) > 0:
                    flow = []
                    flow_id = str(int(img_id)).zfill(6)
                    flow.append(
                        os.path.join(self.data_root, "optflow",sequence,"flo",flow_id + "_fwd.flo")
                    )
                    flow.append(
                        os.path.join(self.data_root, "optflow",sequence,"flo",flow_id + "_bwd.flo")
                    )
                    img_id_2 = int(img_id) - 1
                    stereo_depth_path.append(
                        os.path.join(self.stereo_depth_root, "sequences", sequence, str(img_id_2).zfill(6) + '.npy'))
                scans.append(
                    {"img_2_path": img_2_path,
                     "img_3_path": img_3_path,
                     "sequence": sequence,
