...
class CreateDepthFromLiDAR(object):
    ...
    def __call__(self, results):
        if self.dataset == 'kitti':
            ...
            lidar_filename = os.path.join(self.data_root, 'data_2d_raw', seq_id, 'velodyne_points', 'data',
        ...
        for img_index in range(imgs.shape[0]):
            ...
            gt_depths1.append(gt_depth)

        gt_depths1 = torch.stack(gt_depths1)
        results['gt_depths'] = gt_depths1

        for img_index in range(imgs2.shape[0]):
            gt_depth = torch.zeros((img_h, img_w))
            projected_points_i = projected_points[:, img_index]
            valid_mask_i = valid_mask[:, img_index]
            valid_points_i = projected_points_i[valid_mask_i]
            ...
            gt_depths2.append(gt_depth)

        gt_depths2 = torch.stack(gt_depths2)
        results['gt_depths2'] = gt_depths2
